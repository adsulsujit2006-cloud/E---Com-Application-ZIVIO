...
public void sentLoginOtp(String email, USER_ROLE role) throws Exception {
    ...
    try {
        emailService.getClass()
                .getMethod("sendVerificationOtpEmail", String.class, String.class, String.class, String.class)
                .invoke(emailService, email, otp, subject, text);
    } catch (java.lang.reflect.InvocationTargetException e) {
        Throwable cause = e.getCause();
        if (cause instanceof Exception) {
            throw (Exception) cause;
        }
        throw new Exception("Unable to send verification OTP email", cause);
    } catch (ReflectiveOperationException e) {
        throw new Exception("Unable to send verification OTP email", e);
    }
}
